...
var _ datasource.DataSourceWithConfigure = (*ConnectivityDirectoryServicesDataSource)(nil)

func NewConnectivityDirectoryServicesDataSource() datasource.DataSource {
...
func (d *ConnectivityDirectoryServicesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := ConnectivityDirectoryServicesResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.Connectivity.Directory.Services.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[ConnectivityDirectoryServicesResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
