...
var _ datasource.DataSourceWithConfigure = (*ZeroTrustDevicePostureIntegrationsDataSource)(nil)
...
func (d *ZeroTrustDevicePostureIntegrationsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := ZeroTrustDevicePostureIntegrationsResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.ZeroTrust.Devices.Posture.Integrations.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[ZeroTrustDevicePostureIntegrationsResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
