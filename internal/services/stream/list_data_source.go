...
var _ datasource.DataSourceWithConfigure = (*StreamsDataSource)(nil)
...
func (d *StreamsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := StreamsResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.Stream.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[StreamsResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
