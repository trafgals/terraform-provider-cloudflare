...
var _ datasource.DataSourceWithConfigure = (*LoadBalancersDataSource)(nil)
...
func (d *LoadBalancersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := LoadBalancersResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.LoadBalancers.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[LoadBalancersResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
