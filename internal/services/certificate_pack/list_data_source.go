...
type CertificatePacksDataSource struct {
	client *cloudflare.Client
}

var _ datasource.DataSourceWithConfigure = (*CertificatePacksDataSource)(nil)

func NewCertificatePacksDataSource() datasource.DataSource {
	return &CertificatePacksDataSource{}
}

func (d *CertificatePacksDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *CertificatePacksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := CertificatePacksResultListDataSourceEnvelope{}
	...
	result, diags := customfield.NewObjectListFromAttributes[CertificatePacksResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
