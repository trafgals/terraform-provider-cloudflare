...
func Transform(ctx context.Context, source SourceCloudflareRecordModel) (*TargetDNSRecordModel, diag.Diagnostics) {
...
func transformData(sourceData SourceDataModel, recordType string) (*TargetDNSRecordDataModel, diag.Diagnostics) {
	...
	// Flags: source is String (may contain numeric values or be empty), target is DynamicValue
	// In v4, CAA flags were stored as strings (e.g., "0", "128")
	// The API returns flags as strings, so we keep them as strings in v5 state
	// Non-CAA records may have empty string "" for flags
	if !sourceData.Flags.IsNull() && !sourceData.Flags.IsUnknown() {
		flagsStr := sourceData.Flags.ValueString()
		if flagsStr != "" {
			// Keep as string to match API format (API returns "0", "128", etc.)
			targetData.Flags = customfield.RawNormalizedDynamicValueFrom(types.StringValue(flagsStr))
		}
		// Empty string: don't set flags (let it be null in target)
	}
