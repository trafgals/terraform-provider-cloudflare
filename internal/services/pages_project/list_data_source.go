...
var _ datasource.DataSourceWithConfigure = (*PagesProjectsDataSource)(nil)
...
func (d *PagesProjectsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	env := PagesProjectsResultListDataSourceEnvelope{}
	maxItems := int(data.MaxItems.ValueInt64())
	acc := []attr.Value{}
	if maxItems <= 0 {
		maxItems = 1000
	}
	page, err := d.client.Pages.Projects.List(ctx, params)
	if err != nil {
	...
	result, diags := customfield.NewObjectListFromAttributes[PagesProjectsResultDataSourceModel](ctx, acc)
	resp.Diagnostics.Append(diags...)
	data.Result = result
